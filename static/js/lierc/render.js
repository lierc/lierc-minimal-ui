var Render = function (message, force_raw) {
  ...
  switch (String(message.Command)) {
    ...
    case 'PRIVMSG':
      ...
      if (!name.length || !text.length) { return raw(message) }
  ...
  function raw (message) {
    ...
    if (message.Command.match(/^[45][0-9][0-9]$/)) { text = message.Params.join(' ') } else if (message.Command.match(/^[0-9][0-9][0-9]$/)) { text = message.Params.slice(1).join(' ') } else { text = [message.Prefix.Name, message.Command].concat(message.Params).join(' ') }
