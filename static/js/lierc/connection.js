    ...
    switch (String(message.Command)) {
      ...
      case '353':
        ...
        if (channel) {
          ...
          for (i in nicks) {
            if (nicks[i]) { var nick_mode = conn.nick_mode(nicks[i]) }
      ...
      case 'PRIVMSG':
        ...
        if (name == conn.nick && priv) { fire('private:' + type, conn.id, nick, message) } else if (priv) { fire('private:' + type, conn.id, name, message) } else if (channel) {
