...
var Lierc = function (url, user) {
  ...
  lierc.setup_connection = function (id, host, nick) {
    if (lierc.connections[id]) { return }
    ...
    connection.on('private:msg', function (conn, nick, message) {
      ...
      if (from && (!lierc.is_focused(panel) || !panel.is_scrolled())) { lierc.elem.audio.play() }
    })
    ...
    connection.on('channel:msg', function (conn, channel, message) {
      ...
      if (html) {
        ...
        if (message.Highlight && (!lierc.is_focused(panel) || !panel.is_scrolled())) { lierc.elem.audio.play() }
      }
    })
  ...
  lierc.init = function () {
    ...
    lierc.api.get('/connection', {
      success: function (configs) {
        if (!configs.length) { lierc.config_modal() }
        ...
        if (lierc.stream) { lierc.stream.destroy() }
  ...
  lierc.remove_panel = function (id) {
    ...
    if (focused) {
      if (lierc.last_panel_id && lierc.panels[lierc.last_panel_id]) { lierc.focus_panel(lierc.last_panel_id) } else if (Object.keys(lierc.panels).length) { lierc.focus_panel(Object.keys(lierc.panels)[0]) } else { lierc.focused = null }
    }
  ...
  lierc.add_panel = function (name, connection, focus) {
    ...
    if (lierc.panels[id]) { return lierc.panels[id] }
    ...
    if (lierc.last_seen[panel.id]) { panel.last_seen = lierc.last_seen[panel.id] }
    panel.update_nav()
    ...
    if (panel.type == 'status') { lierc.elem.status.append(panel.elem.nav) } else if (panel.type == 'private') { lierc.elem.privates.append(panel.elem.nav) } else { lierc.insert_sorted_nav(panel) }

    lierc.update_nav_counts()

    if (panel.type == 'channel') { lierc.ignore_events_pref(panel) }
    ...
    panel.elem.nav.addEventListener('click', function (e) {
      ...
      if (e.target.classList.contains('close-panel')) {
        if (panel.type == 'channel') { lierc.part_channel(panel.name, panel.connection) } else if (panel.type == 'status') { lierc.delete_connection(panel.connection) } else { lierc.close_panel(id) }
    ...
    if (!panel.focused && lierc.missed[panel.id]) { lierc.apply_missed(panel, lierc.missed[panel.id]) }
  ...
  lierc.channel_panels = function () {
    var panels = 0
    for (id in lierc.panels) {
      if (lierc.panels[id].type == 'channel') { panels++ }
    }
    return panels
  }
    ...
    lierc.api.get('/' + parts.join('/'), {
      ...
      success: function (events) {
        ...
        events.forEach(function (e) {
          ...
          if (lierc.is_reaction(message)) { reactions.push(message) } else {
  ...
  lierc.focus_panel = function (id) {
    ...
    if (panel.first_focus && panel.type == 'channel') { lierc.show_nicklist_pref(panel) }
  ...
  lierc.sync_missed = function () {
    if (lierc.focused) {
      ...
      if (lierc.focused.last_seen) { lierc.last_seen[lierc.focused.id] = lierc.focused.last_seen }
    }
  ...
  lierc.focus_input = function (force) {
    ...
    if (lierc.mobile) { return }
    if (!lierc.focused) { return }
    if (lierc.overlayed()) { return }
    lierc.focused.elem.input.focus()
  }
  ...
  lierc.close_dialog = function () {
    if (lierc.dialog) { lierc.dialog.close() }
    lierc.dialog = null
  }
