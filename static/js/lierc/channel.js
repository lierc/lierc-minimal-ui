var Channel = function (name) {
  ...
  this.set_mode = function (param) {
    ...
    for (var i = 0; i < modes.length; i++) {
      ...
      var mode = modes[i]
      if (action == '+' && current.indexOf(mode) == -1) { this.mode = current + mode } else if (action == '-' && current.indexOf(mode) != -1) { this.mode = current.replace(mode, '') }
    }
  }
  ...
  this.nick_mode = function (nick, param) {
    ...
    for (var i = 0; i < modes.length; i++) {
      ...
      var mode = modes[i]
      if (action == '+' && current.indexOf(mode) == -1) { this.nick_modes[nick] = current + mode } else if (action == '-' && current.indexOf(mode) != -1) { this.nick_modes[nick] = current.replace(mode, '') }
    }
  }
