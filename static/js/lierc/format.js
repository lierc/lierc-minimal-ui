...
Unformat.descend = function (node, string) {
  if (node.nodeType == 3) {
    ...
    if (Unformat.block.indexOf(node.nodeName) != -1) { string += '\n' }
    if (Unformat.tags[node.nodeName]) { string += Unformat.tags[node.nodeName] }
    if (node.nodeType == 1) {
      ...
      if (node.hasAttribute('data-color-reset')) { string += '\x03' }
    }
...
var Format = function (text) {
  ...
  return Format.parse([], tokens).filter(function (item) {
    ...
    if (item.background != null) { span.style.backgroundColor = Format.color_map[parseInt(item.background)] }
    if (item.color != null) { span.style.color = Format.color_map[parseInt(item.color)] }
    if (item.bold) { span.style.fontWeight = 'bold' }
    if (item.italic) { span.style.fontStyle = 'italic' }
    if (item.underline) { span.style.textDecoration = 'underline' }
