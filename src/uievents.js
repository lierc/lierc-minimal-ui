var UIEvents = function (lierc) {
  ...
  lierc.elem.switcher.addEventListener('input', function (e) {
    ...
    for (var i = 0; i < items.length; i++) {
      if (items[i].classList.contains('selected')) {
        if (items[i].classList.contains('match')) { return }
  ...
  document.addEventListener('keyup', function (e) {
    if (e.which == 18) { mods['meta'] = false }
    if (e.which == 16) { mods['shift'] = false }
    if (e.which == 17) { mods['ctrl'] = false }
    if (e.which == 91) { mods['cmd'] = false }
  })
  ...
  function send_submit () {
    ...
    var value = Unformat.irc_text(input.innerHTML)
    if (value == '') return
    input.innerHTML = ''
    ...
    function sendlines (lines) {
      ...
      lierc.api.post('/connection/' + panel.connection, {
        ...
        success: function (res) {
          lierc.post_tokens.push(res.token)
          if (lines.length) { sendlines(lines) }
        },
        error: function (e) {
          ;
          alert('Error: ' + e)
  ...
  document.addEventListener('copy', function (e) {
    ...
    for (i in selected) {
      ...
      if (line.match(/^\d\d:\d\d$/)) { continue }
      ...
      if (line.match(/\S/)) { lines.push(line) }
    }
  ...
  document.addEventListener('paste', function (e) {
    if (lierc.overlayed()) { return }
    if (document.activeElement.matches('#gist-upload-input')) { return }
  ...
  lierc.elem.panel.addEventListener('click', function (e) {
    if (!e.target.matches('.message-menu, .message-menu *')) { return }
  ...
  lierc.elem.panel.addEventListener('click', function (e) {
    if (!e.target.matches('.embed-toggle')) { return }
  ...
  lierc.elem.panel.addEventListener('click', function (e) {
    if (!e.target.matches('.message-react, .message-react *')) { return }
  ...
  lierc.elem.panel.addEventListener('transitionend', function (e) {
    if (e.target.matches('li.chat')) { e.target.classList.remove('loading', 'loaded') }
  })
}
